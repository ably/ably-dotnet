...
MONO_MACOS_PKG_DOWNLOAD_URL='https://download.mono-project.com/archive/6.12.0/macos-10-universal/MonoFramework-MDK-6.12.0.206.macos10.xamarin.universal.pkg'

# create a temp dir and cd into it
mkdir -p /tmp/mono-install
cd /tmp/mono-install

# debug: mono version before the install (if it exists)
if command -v mono &> /dev/null; then
    echo "Mono is already installed:"
    mono --version
else
    echo "Mono is not currently installed"
fi
