...
using IO.Ably.TestHelpers.Unity;
using IO.Ably.Tests;
using IO.Ably.Types;
...
{
    [TestFixture]
    public class ChannelSandboxSpecs
    {
        ...
        [Theory]
        public async Task TestGetChannel_ReturnsValidChannel(Protocol protocol = Protocol.Json)
        {
            // Arrange
            var client = await UnitySandbox.GetRealtimeClient(protocol);
        ...
        public async Task TestAttachChannel_AttachesSuccessfully(Protocol protocol = Protocol.Json)
        {
            // Arrange
            var client = await UnitySandbox.GetRealtimeClient(protocol);
        ...
        [Property("spec", "RTL1")]
        public async Task SendingAMessageAttachesTheChannel_BeforeReceivingTheMessages(Protocol protocol = Protocol.Json)
        {
            // Arrange
            var client = await UnitySandbox.GetRealtimeClient(protocol);
            IRealtimeChannel target = client.Channels.Get("test");
        ...
        {
            var client = await UnitySandbox.GetRealtimeClient(protocol, (options, settings) =>
            {
        ...
        [Property("spec", "RTL4j")]
        public async Task WhenChannelIsAlreadyAttached_AndReAttachIsForcedByChangingChannelOptions_ShouldPassAttachResumeFlagInAttachMessage(Protocol protocol = Protocol.Json)
        {
            var sentMessages = new List<ProtocolMessage>();
            var client = await UnitySandbox.GetRealtimeClient(protocol, (options, _) =>
            {
        ...
        [Property("spec", "RTL4j2")]
        public async Task TestAttachResume_And_RewindParam(Protocol protocol = Protocol.Json)
        {
            var client = await UnitySandbox.GetRealtimeClient(protocol);
            var client1 = await UnitySandbox.GetRealtimeClient(protocol, (opts, _) => opts.AutoConnect = false);
            var client2 = await UnitySandbox.GetRealtimeClient(protocol, (opts, _) => opts.AutoConnect = false);
        ...
        [Property("spec", "RTL4k1")]
        public async Task ChannelParamsIncludedInTheAttachedMessage_ShouldBeExposedAsReadonlyOnChannel(Protocol protocol = Protocol.Json)
        {
            var client = await UnitySandbox.GetRealtimeClient(protocol);
        ...
        [Property("spec", "RTL4m")]
        public async Task ChannelModesIncludedInTheAttachedMessage_ShouldBeExposedAsReadonlyOnChannel(Protocol protocol = Protocol.Json)
        {
            var client = await UnitySandbox.GetRealtimeClient(protocol);
        ...
        [Property("spec", "RTL16a")]
        public async Task SetOptions_WithDifferentModesOrParams_ShouldReAttachChannel(Protocol protocol = Protocol.Json)
        {
            var client = await UnitySandbox.GetRealtimeClient(protocol);
            ...
            Debug.WriteLine(states.Select(x => x.Item1.ToString()).JoinStrings());
        ...
        [Property("spec", "RTC1a")]
        public async Task TestAttachChannel_Sending3Messages_EchoesItBack(Protocol protocol = Protocol.Json)
        {
            // Arrange
            var client = await UnitySandbox.GetRealtimeClient(protocol);
        ...
        [Property("spec", "RTL7f")]
        [Property("spec", "RTC1a")]
        public async Task TestAttachChannel_SendingMessage_DoesNot_EchoesItBack(Protocol protocol = Protocol.Json)
        {
            ...
            // Arrange
            var client = await UnitySandbox.GetRealtimeClient(protocol, (o, _) =>
            {
        ...
        [Theory]
        [Property("spec", "RTL6b")]
        public async Task With3ClientsAnd60MessagesAndCallbacks_ShouldExecuteAllCallbacks(Protocol protocol = Protocol.Json)
        {
            ...
            while (retry)
            {
                var client1 = await UnitySandbox.GetRealtimeClient(protocol);
                var client2 = await UnitySandbox.GetRealtimeClient(protocol);
                var client3 = await UnitySandbox.GetRealtimeClient(protocol);
        ...
        [Theory]
        [Property("spec", "RTL6c1")]
        public async Task TransientPublishing_WhenConnected_ShouldPublishWithoutAttemptingAttach(Protocol protocol = Protocol.Json)
        {
            var channelName = "RTL6c1".AddRandomSuffix();
            var pubClient = await UnitySandbox.GetRealtimeClient(protocol);
            var subClient = await UnitySandbox.GetRealtimeClient(protocol);
        ...
        [Theory]
        [Property("spec", "RTL6c1")]
        public async Task TransientPublishing_WhenConnecting_ShouldPublishWithoutAttemptingAttach(Protocol protocol = Protocol.Json)
        {
            var channelName = "RTL6c1".AddRandomSuffix();
            var subClient = await UnitySandbox.GetRealtimeClient(protocol);
            await subClient.WaitForState(ConnectionState.Connected);
        ...
        [Theory]
        [Property("spec", "RTL6c2")]
        public async Task TransientPublishing_WhenNotConnected_ShouldPublishWithoutAttemptingAttach(Protocol protocol = Protocol.Json)
        {
            var channelName = "RTL6c2".AddRandomSuffix();
            var pubClient = await UnitySandbox.GetRealtimeClient(
                                protocol,
        ...
        [Theory]
        [Property("spec", "RTL6c4")]
        public async Task TransientPublishing_WhenConnectionFailed_ShouldResultInError(Protocol protocol = Protocol.Json)
        {
        ...
        [Property("spec", "RTL6c4")]
        public async Task TransientPublishing_WhenChannelFailed_ShouldResultInError(Protocol protocol = Protocol.Json)
        {
            var channelName = "RTL6c4".AddRandomSuffix();
        ...
        [Property("spec", "RTL6c5")]
        public async Task PublishShouldNotImplicitlyAttachAChannel(Protocol protocol = Protocol.Json)
        {
            var client = await UnitySandbox.GetRealtimeClient(protocol);
        ...
        [Property("spec", "RTL6e")]
        [Property("spec", "RTL6e1")]
        public async Task WithBasicAuthAndAMessageWithClientId_ShouldReturnTheMessageWithThatClientID(Protocol protocol = Protocol.Json)
        {
            var client = await UnitySandbox.GetRealtimeClient(protocol);
            ...
            channel.Publish(new Message("test", "withClientId") { ClientId = "123" });

            UnitySandbox.ResetEvent.WaitOne(4000).Should().BeTrue("Operation timed out");
        ...
        [Property("spec", "RTL6g1b")]
        public async Task WithAClientIdInOptions_ShouldReceiveMessageWithCorrectClientID(Protocol protocol = Protocol.Json)
        {
            var clientId = (new Random().Next(1, 100000) % 1000).ToString();
            var client = await UnitySandbox.GetRealtimeClient(protocol, (opts, _) => opts.ClientId = clientId);
        ...
        [Property("spec", "RTL6g1b")]
        public async Task WithAnImplicitClientIdFromToken_ShouldReceiveMessageWithCorrectClientID(Protocol protocol = Protocol.Json)
        {
            var rest = await UnitySandbox.GetRestClient(protocol);
            var token = await rest.Auth.RequestTokenAsync(new TokenParams { ClientId = "1000" });
            var client = await UnitySandbox.GetRealtimeClient(protocol, (opts, _) => opts.TokenDetails = token);
        ...
        [Property("spec", "RTL6g2")]
        public async Task WithAClientIdInOptionsAndMatchingClientIdInMessage_ShouldSendAndReceiveMessageWithCorrectClientID(Protocol protocol = Protocol.Json)
        {
            var client = await UnitySandbox.GetRealtimeClient(protocol, (opts, _) => opts.ClientId = "999");
            ...
            channel.Publish(new Message("test", "data") { ClientId = "999" });
            UnitySandbox.ResetEvent.WaitOne(4000).Should().BeTrue("Timed out");
        ...
        [Property("spec", "RTL6g2")]
        public async Task WithAClientIdInOptionsAndDifferentClientIdInMessage_ShouldNotSendMessageAndResultInAnError(Protocol protocol = Protocol.Json)
        {
            var client = await UnitySandbox.GetRealtimeClient(protocol, (opts, _) => opts.ClientId = "999");
        ...
        [Property("spec", "RTL6g4")]
        [Property("spec", "RTL6g3")]
        [Property("spec", "RTL6h")] // Look at PublishAsync
        public async Task
        ...
        [Theory]
        [Property("spec", "RTL6f")]
        public async Task ConnectionIdShouldMatchThatOfThePublisher(Protocol protocol = Protocol.Json)
        {
            var client = await UnitySandbox.GetRealtimeClient(protocol, (opts, _) => opts.ClientId = "999");
            var channelName = "test".AddRandomSuffix();
            var channel = client.Channels.Get(channelName);
            ...
            channel.Publish(new Message("test", "best"));
            UnitySandbox.ResetEvent.WaitOne();
        ...
        [Property("spec", "RTL6f")]
        public async Task PublishedMessagesShouldContainMessageIdsWhenReceived(Protocol protocol = Protocol.Json)
        {
            var client = await UnitySandbox.GetRealtimeClient(protocol, (opts, _) => opts.ClientId = "999");
            var channelName = "test".AddRandomSuffix();
            var channel = client.Channels.Get(channelName);
...
#pragma warning restore 162
        ...
        [Theory]
        [Ignore("Detached, Failed and Suspended")]
        ...
        {
            var client = await UnitySandbox.GetRealtimeClient(Defaults.Protocol, (options, settings) =>
                {
        ...
        {
            var client = await UnitySandbox.GetRealtimeClient(Defaults.Protocol);
            var channel = client.Channels.Get("test".AddRandomSuffix());
        ...
        {
            var client1 = await UnitySandbox.GetRealtimeClient(Protocol.Json);
        ...
        [Theory]
        [Property("spec", "RTL10d")]
        public async Task WithOneClientPublishingAnotherShouldBeAbleToRetrieveMessages(Protocol protocol = Protocol.Json)
        {
            var client1 = await UnitySandbox.GetRealtimeClient(protocol);
        ...
        [Property("spec", "RTL13a")]
        public async Task ServerInitiatedDetach_WhenChannelAttached_ShouldReattachImmediately(Protocol protocol = Protocol.Json)
        {
            ...
            await UnitySandbox.WaitFor(done =>
            {
        ...
        [Property("spec", "RTL13a")]
        public async Task ServerInitiatedDetach_WhenChannelSuspended_ShouldReattachImmediately(Protocol protocol = Protocol.Json)
        {
            var channelName = "RTL13a".AddRandomSuffix();
            var client = await UnitySandbox.GetRealtimeClient(protocol);
            client.Connect();
        ...
        [Theory]
        [Property("spec", "RTL13b")]
        public async Task ServerInitiatedDetach_WhenChannelAttached_ShouldAttemptReattachImmediately_WhenReattachFailsBecomeSuspended(Protocol protocol = Protocol.Json)
        {
            ...
            await UnitySandbox.WaitFor(30000, done =>
            {
        ...
        [Property("spec", "RTL13b")]
        [Property("spec", "RTL13c")]
        public async Task ServerInitiatedDetach_WhenChannelAttaching_ShouldAttemptReattachImmediately_WhenReattachFailsBecomeSuspended(Protocol protocol = Protocol.Json)
        {
            // reduce timeouts to speed up test
            var requestTimeout = TimeSpan.FromSeconds(2);
            var client = await UnitySandbox.GetRealtimeClient(protocol, (options, settings) =>
        ...
        [Property("issue", "117")]
        public async Task AttachAwaitShouldTimeoutIfStateChanges(Protocol protocol = Protocol.Json)
        {
            var client1 = await UnitySandbox.GetRealtimeClient(protocol, (opts, _) =>
            {
                opts.AutoConnect = false;
        ...
        [Property("issue", "104")]
        public async Task AttachWithMultipleConcurrentClientsShouldWork(Protocol protocol = Protocol.Json)
        {
            var clients = new List<IRealtimeClient>
            {
                await UnitySandbox.GetRealtimeClient(protocol, (opts, _) => opts.AutoConnect = false),
                await UnitySandbox.GetRealtimeClient(protocol, (opts, _) => opts.AutoConnect = false),
                await UnitySandbox.GetRealtimeClient(protocol, (opts, _) => opts.AutoConnect = false),
                await UnitySandbox.GetRealtimeClient(protocol, (opts, _) => opts.AutoConnect = false),
                await UnitySandbox.GetRealtimeClient(protocol, (opts, _) => opts.AutoConnect = false)
        ...
        [Property("issue", "116")]
        public async Task FailureOfHistoryApiCallMeansChannelsNoLongerAttach(Protocol protocol = Protocol.Json)
        {
            var client = await UnitySandbox.GetRealtimeClient(protocol, (opts, _) => opts.AutoConnect = false);
        ...
        [Property("issue", "205")]
        public async Task ShouldReturnToPreviousStateIfAttachMessageNotReceivedWithinDefaultTimeout2(Protocol protocol = Protocol.Json)
        {
            var client = await UnitySandbox.GetRealtimeClient(protocol);
        ...
        public async Task WhenAttachAsyncCalledAfterSubscribe_ShouldWaitUntilChannelIsAttached(Protocol protocol = Protocol.Json)
        {
            var client = await UnitySandbox.GetRealtimeClient(protocol);
        ...
        public async Task WhenSubscribeHandlerBlocks_ShouldBehaveOk(Protocol protocol = Protocol.Json)
        {
            var client = await UnitySandbox.GetRealtimeClient(protocol);
            var channel = client.Channels.Get("test".AddRandomSuffix());
